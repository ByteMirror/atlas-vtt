...
- **Token UI scales with its token**: resource bars, nameplates, condition markers, +/- buttons and resize/rotate handles live in world space and are scaled by `tokenUIScale(spriteSize)` (`src/app/pixi/token-renderer/tokenSizing.ts`; 1 for a medium token on a 70px grid), like Owlbear Rodeo. Put an anchor container on the token's edge in world units and lay the UI out in UI units inside it. Never keep token UI at a constant screen size: it then hides small tokens and looks tiny on large ones.
